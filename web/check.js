  ...
  async function renderInner(parsed, target, depth) {
    ...
    for (const c of inner) {
      ...
      rows.push(h('div', { class: 'move' }, [
        ...
        await renderInner(p, t, (depth || 0) + 1),
      ]));
    }
    ...
    if (s.startsWith('{') || s.startsWith('[')) {
      try {
        ...
        const calls = list
          ...
          .filter((o) => o && (o.to || o.data))
          .map((o) => ({ to: o.to, data: o.data || o.input || '0x', from: o.from, value: o.value }));
        if (calls.length === 1) return Object.assign({ kind: 'call' }, calls[0]);
        if (calls.length > 1) return { kind: 'batch', calls };
    ...
    try {
      ...
      if (parsedInput.kind === 'batch') {
        ...
        for (let i = 0; i < parsedInput.calls.length && i < 20; i++) {
          ...
          if (!ethers.isAddress(String(c.to || ''))) { cards.push(note('bad', 'Call ' + (i + 1) + ' has no valid destination', String(c.to || '(none)'))); continue; }
          const t = await readAddress(c.to).catch(() => null);
          if (stale(seq)) return;
          const p = parseData(c.data, t && t.abi);
          let sim = null; try { sim = await simulate({ to: c.to, data: c.data, from: from || c.from, value: c.value }); } catch (e) {}
          if (stale(seq)) return;
          const sentence = p.unknown || p.empty ? null : describeCall(p, { target: t });
