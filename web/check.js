(() => {
  ...
  function readEnvelope(o, note, opts) {
    ...
    if (!loose) {
      ...
      else if (!isHexQuantity(o.chainId)) schema.push('Its chainId is ' + typeName(o.chainId) + '. This field has to be a hex string such as "0xb626", so a wallet may refuse the request outright.');
    ...
    for (let i = 0; i < o.calls.length; i++) {
      ...
      calls.push({ to: (typeof c.to === 'string' && /^0x[0-9a-fA-F]{40}$/.test(c.to)) ? c.to : null,
                   data: HEX.test(data) ? data : '0x', from: null, value: c.value, invalid: bad });
    }
    return {
      envelope: {
          ...
          for (let i = 0; i < calls.length && i < 20; i++) {
            ...
            if (!ethers.isAddress(String(c.to || ''))) {
              cards.push(card(title, [
                c.invalid
                  ? note('bad', 'This entry could not be read as a call', c.invalid
                      + ' It is shown here at its own position so the list you see is the list you pasted.')
                  : note('bad', 'This call has no destination this page can read',
                      'An entry with no `to` is usually a contract being created, and this page cannot tell you what that contract would do.'),
