(() => {
  ...
  function decodeRevert(data, iface) {
    ...
    try {
      if (sel === '0x08c379a0') return '\u201c' + ethers.AbiCoder.defaultAbiCoder().decode(['string'], '0x' + data.slice(10))[0] + '\u201d';
      if (sel === '0x4e487b71') {
  ...
  async function readAddress(addr) {
    ...
    out.code = await p.getCode(out.address).catch(() => '0x');
    out.isContract = out.code !== '0x';
    ...
    if (out.proxy && out.proxy.target) {
      const impl = (await explorerJson('/smart-contracts/' + out.proxy.target)).data;
      if (impl) { out.proxy.verified = !!impl.is_verified; out.proxy.name = impl.name || null; if (impl.abi && !out.abi) out.abi = impl.abi; if (impl.abi) out.implAbi = impl.abi; }
    }
  ...
  function knownFunctions(o) {
    ...
    if (o.abi) {
      ...
      return { list: found, source: 'the published source', long: 'the published source' };
    }
    const sels = selectorsIn(o.code);
    for (const s of sels) {
    ...
    return { list: found, source: 'its bytecode', long: 'the contract’s own bytecode' };
  }
  ...
  function describeCall(parsed, ctx) {
    ...
    switch (parsed.signature) {
      ...
      case 'multicall(bytes[])': return 'Run ' + a[0].length + ' separate calls on ' + what + ' in one transaction. Each one is listed below.';
  ...
  function parseData(data, abi) {
    ...
    const tryWith = (iface, source) => {
      try {
        ...
        if (tx) return { name: tx.name, signature: tx.fragment.format('sighash'), args: tx.args, fragment: tx.fragment, source };
      } catch (e) {}
      return null;
    };
    if (abi) { const r = tryWith(new ethers.Interface(abi), 'the contract’s published source'); if (r) return r; }
    const r = tryWith(KNOWN_IFACE, 'a standard this page recognises');
    if (r) return r;
  ...
  function renderMovements(moves, me, senderLabel) {
    if (!moves.length) return h('p', { class: 'mut', text: 'No tokens and no ETH move.' });
  ...
  async function simulate(call) {
    const params = [{
      blockStateCalls: [{ calls: [{
        from: call.from || ethers.ZeroAddress,
        to: call.to,
        data: call.data || '0x',
        value: call.value ? ethers.toBeHex(BigInt(call.value)) : '0x0',
      }] }],
      traceTransfers: true,
      validation: false,
    }, 'latest'];
    const res = await rp().send('eth_simulateV1', params);
  ...
  async function showTransaction(hash, viewer) {
    ...
    out(
      card(null,
        ...
        h('div', {}, [
          ...
          parsed.source ? h('span', { class: 'pill', text: 'decoded from ' + parsed.source }) : null,
        ]),
      ...
      card('What moved', renderMovements(moves, viewer || (tx && tx.from), viewer ? null : 'the sender’s wallet')),
      parsed && parsed.args && parsed.args.length ? card('Everything it was asked to do', renderArgs(parsed, target)) : null,
  ...
  function contractCard(o, title) {
    ...
    return card(title || 'The contract', [
      h('div', {}, [
        h('span', { class: 'pill ' + (o.verified ? 'ok' : o.verified === null ? 'warn' : 'bad'),
          text: o.verified ? (o.partial ? 'source published, partly matched' : 'source published and matched')
      ...
      kv([
        ...
        o.proxy ? ['Proxy', h('span', {}, document.createTextNode(o.proxy.kind + ' → '), link(o.proxy.target, short(o.proxy.target)), document.createTextNode(o.proxy.verified === false ? '  (that code has no published source either)' : ''))] : null,
      ...
      powers.length
        ? h('div', {}, [
            h('p', { class: 'mut', text: 'What whoever controls this contract can do, read from ' + fns.long + ':' }),
            h('ul', { class: 'plain' }, powers.map((p) => h('li', { class: p.cls === 'mut' ? '' : p.cls }, document.createTextNode(p.what + ' '), h('span', { class: 'mut mono', text: '(' + p.sig + ')' })))),
            o.owner && o.owner !== ethers.ZeroAddress ? h('p', { class: 'mut', text: 'Those powers sit with ' + short(o.owner) + ' today, and can be handed on.' }) : null,
            o.owner === ethers.ZeroAddress ? h('p', { class: 'ok', text: 'Ownership has been given up, so the owner-only ones can no longer be used by anybody.' }) : null,
          ])
        : h('p', { class: o.verified ? 'ok' : 'mut', text: 'Nothing in ' + fns.source + ' lets anyone mint, pause, block, retax or replace this contract.' + (o.verified ? '' : ' That is what the selectors in the code show, which is a floor rather than a ceiling.') }),
      o.verified === false ? note('warn', 'No source has been published',
        'The bytecode is public and this page has read it, but nobody has shown which source code produced it. Everything above about what it can do comes from the function selectors in the code itself, which is a floor and not a ceiling.') : null,
  ...
  async function showCall(to, data, from, value) {
    ...
    if (!target.isContract && !target.delegated)
    ...
    out(
      ...
      sim ? card('What it would move', renderMovements(moves, from)) : null,
      parsed.args && parsed.args.length ? card('Everything it is asking for', renderArgs(parsed, target)) : null,
      parsed.unknown ? card('The raw call', [
        kv([['Selector', mono(parsed.selector)], ['Length', String((data.length - 10) / 2) + ' bytes of arguments']]),
        h('pre', { text: (parsed.rest.match(/.{1,64}/g) || []).join('\n') }),
      ]) : null,
  ...
  function readInput(raw) {
    ...
    if (s.startsWith('{') || s.startsWith('[')) {
      try {
        const j = JSON.parse(s); const o = Array.isArray(j) ? j[0] : (j.params ? j.params[0] : j);
        if (o && (o.to || o.data)) return { kind: 'call', to: o.to, data: o.data || o.input || '0x', from: o.from, value: o.value };
  ...
  async function go() {
    ...
    try {
      ...
      if (parsedInput.kind === 'tx') return await showTransaction(parsedInput.hash, from);
      if (parsedInput.kind === 'address') return await showAddress(parsedInput.address);
      if (parsedInput.kind === 'call') {
        if (!ethers.isAddress(parsedInput.to || '')) { say('That call has no valid "to" address in it.', 'bad'); return; }
        return await showCall(ethers.getAddress(parsedInput.to), parsedInput.data, from || (ethers.isAddress(parsedInput.from || '') ? ethers.getAddress(parsedInput.from) : null), parsedInput.value);
      }
      if (parsedInput.kind === 'data') {
        ...
        return await showCall(ethers.getAddress(to.trim()), parsedInput.data, from, null);
      }
    } catch (e) {
  ...
  $('net').addEventListener('change', () => { $('netPill').textContent = cfg().name; $('out').textContent = ''; say(''); refreshFoot(); });
  ...
  window.__check = { readInput, parseData, describeCall, decodeRevert, selectorsIn, powersOf, knownFunctions };
})();
