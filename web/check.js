  ...
  function readInput(raw) {
    ...
    if (s.startsWith('{') || s.startsWith('[')) {
      try {
        ...
        if (!requests.length) return { kind: 'bad', why: 'That JSON has no calls in it.' };
        if (requests.length === 1 && requests[0].calls.length === 1 && requests[0].calls[0].to && !requests[0].envelope)
          return Object.assign({ kind: 'call' }, requests[0].calls[0]);
        return { kind: 'batch', requests };
    ...
    try {
      ...
      if (parsedInput.kind === 'batch') {
        ...
        for (let ri = 0; ri < reqs.length; ri++) {
          ...
          if (env) cards.push(note(env.atomicRequired ? 'warn' : '', label + (env.atomicRequired ? 'all of it, or none of it' : 'not required to be atomic'),
            env.atomicRequired
