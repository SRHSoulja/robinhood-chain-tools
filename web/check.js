(() => {
  ...
  function innerCalls(parsed, target) {
    ...
    if (parsed.signature === 'upgradeToAndCall(address,bytes)') push(parsed.args[0], parsed.args[1]);
    return out;
  }
    ...
    return card(title || 'The contract', [
      ...
      powers.length
        ? h('div', {}, [
            ...
            o.owner === ethers.ZeroAddress ? h('p', { class: 'mut', text: 'A function called owner() answers the zero address. That often means ownership was given up, but plenty of contracts gate privileged functions on roles or their own rules instead, so on its own it does not mean nobody can use them.' }) : null,
          ])
      ...
      try {
        ...
        const unwrap = (o) => {
          if (o && Array.isArray(o.calls)) {
            if (o.chainId || o.atomicRequired !== undefined) envelope = { chainId: o.chainId, atomicRequired: !!o.atomicRequired, from: o.from };
            return o.calls.map((c) => Object.assign({ from: o.from }, c));
          }
          return [o];
        };
        ...
        if (calls.length === 1 && calls[0].to && !envelope) return Object.assign({ kind: 'call' }, calls[0]);
        if (calls.length >= 1) return { kind: 'batch', calls, envelope };
  ...
  async function go() {
    ...
    try {
      ...
      if (parsedInput.kind === 'batch') {
        const env = parsedInput.envelope;
        // B-03: the request names the chain it is for. Answering about the same address on a different chain
        ...
        if (env && env.chainId) {
          ...
          if (want !== null && want !== chainId()) {
            say('');
            out(note('bad', 'This request is for a different network',
              'It names chain ' + want + (CHAINS[want] ? ' (' + CHAINS[want].name + ')' : '') + ', and this page is set to '
              + chainId() + ' (' + cfg().name + '). Nothing has been checked, because the same address is a different contract on a different chain, and an answer from the wrong one would be worse than no answer. Change the network above'
              + (CHAINS[want] ? ' to ' + CHAINS[want].name + ' and try again.' : '; this page does not cover that chain.')));
            return;
          }
        }
        say('Reading all ' + parsedInput.calls.length + ' calls…');
        const cards = [note('warn', 'This is ' + parsedInput.calls.length + ' calls, not one',
        ...
        if (env) cards.push(note(env.atomicRequired ? 'warn' : '', env.atomicRequired ? 'All of it, or none of it' : 'Not required to be atomic',
          env.atomicRequired
            ? 'The request asks for these to happen together. If one fails, the wallet is being asked to make sure none of them happen.'
            : 'The request does not ask for these to happen together, so some may land and others may not.'));
        // B-04: run them in order, as the wallet would, so a call that only fails because of an earlier one
        // is seen here rather than after signing.
        let ordered = null, orderedErr = null;
        try { ordered = await simulateInOrder(parsedInput.calls.filter(c => ethers.isAddress(String(c.to || '')))); }
        catch (e) { orderedErr = String(e.message || e).slice(0, 140); }
        if (stale(seq)) return;
        if (ordered) {
          const bad = ordered.findIndex(r => !r.ok);
          if (bad >= 0) cards.push(note('bad', 'Run in order, call ' + (bad + 1) + ' fails',
            decodeRevert(ordered[bad].reason, null) + (env && env.atomicRequired
              ? '  Because this request asks for all or nothing, that means none of it would happen.'
              : '  The calls before it would still happen.')));
          else cards.push(note('ok', 'Run in order, every call succeeds', 'Simulated as one sequence against the chain as it is now, which is how the wallet would send them.'));
        } else {
          cards.push(note('warn', 'These could not be run in order', (orderedErr || 'the node would not do it')
            + '. Each call below was checked on its own instead, against the chain as it is now. A call that only fails because of what an earlier one did would not be caught by that.'));
        }
        for (let i = 0; i < parsedInput.calls.length && i < 20; i++) {
          ...
          if (!ethers.isAddress(String(c.to || ''))) {
            cards.push(card('Call ' + (i + 1) + ' of ' + parsedInput.calls.length, [
              note('bad', 'This call has no destination this page can read',
                'A request entry with no `to` is usually a contract being created, and this page cannot tell you what that contract would do. It is part of the same request as the others, so nothing here is the whole picture.'),
          ...
          if (stale(seq)) return;
          const p = parseData(c.data, t && t.abi);
          // S-02: the sender the request itself names, when the box above is blank; and a simulation that
          // failed is reported rather than swallowed.
          const sender = from || (ethers.isAddress(String(c.from || '')) ? ethers.getAddress(c.from) : null);
          let sim = ordered ? ordered[i] : null, simErr = null;
          if (!sim) { try { sim = await simulate({ to: c.to, data: c.data, from: sender, value: c.value }); } catch (e) { simErr = String(e.message || e).slice(0, 140); } }
          if (stale(seq)) return;
          const sentence = p.unknown || p.empty ? null : describeCall(p, { target: t });
          cards.push(card('Call ' + (i + 1) + ' of ' + parsedInput.calls.length, [
            ...
            ...callWarnings(p, t, sim, sender).map(([cls, title, body]) => note(cls, title, body)),
            t ? kv([
