(() => {
  ...
  async function readAddress(addr) {
    ...
    // Round twenty F-2: explorerJson decided "this is a real answer" from the HTTP status and content-type
    // alone, so a 200 JSON body that is not a Blockscout contract record at all -- a rate-limit envelope, a
    // gateway's own error document, an explorer that answers "not found" in the body rather than the status --
    // was returned as data, and `!!sc.is_verified` on a record with no such field read as a definite `false`:
    // "no source published", stated as fact. Only a real 404 (sc === null) may say that; anything else that
    // does not carry a real `is_verified` boolean was not an answer to this question at all.
    else if (typeof sc.is_verified !== 'boolean') { out.verified = null; out.explorerDown = true; }
    if (sc && typeof sc.is_verified === 'boolean') {
      ...
      // Round twenty F-1: the mainnet Worker deliberately answers `is_partially_verified: null` when the
      // module API it reads does not carry the field ("null, not a claim"). `!!null` is `false`, so this used
      // to read "not known" as "not partial" and the pill said "matched" about a verification that could be
      // full or partial. Three states now, matching what the Worker actually knows.
      out.partial = sc.is_partially_verified === true ? true
        : (sc.is_partially_verified === null || sc.is_partially_verified === undefined ? null : false);
  ...
  function contractCard(o, title) {
    ...
    return card(title || 'The contract', [
      h('div', {}, [
        ...
        h('span', { class: 'pill ' + (o.proxy
          ...
          text: o.proxy
            ...
            : (o.verified ? (o.partial === true ? 'source published, partly matched'
                : o.partial === null ? 'source published (whether the match is full or partial is not known)'
                : 'source published and matched')
      ...
      h('div', { class: 'note warn' }, [
        ...
        o.partial === true ? document.createTextNode('  This contract is only partially verified, so even the published source may not be all of the code that runs.')
          : o.partial === null ? document.createTextNode('  Whether this match is full or partial is not known, so even the published source may not be all of the code that runs.') : null,
      ]),
