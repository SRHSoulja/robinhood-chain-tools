(() => {
  ...
  // One reading of what an explorerJson('/smart-contracts/<addr>') answer says about verification, used for
  // the contract itself and for a proxy's implementation alike. Round 21 F-1/F-2: this question used to be
  // answered twice -- once here (round twenty F-2) and once, separately and older, at the proxy's own lookup
  // -- and the two disagreed: a 200 body that is not a real Blockscout record (a rate-limit envelope, a
  // gateway error page, an explorer that reports "not found" in the body rather than the status) is not an
  // answer to "is the source published" and must read as could-not-check, not as a false, or falsely
  // confident, one. Three states -- verified true, false, or null -- plus, only once verified is known,
  // whether the match is full, partial, or (mainnet) not knowable at all.
  function verificationOf(res) {
    if (!res || !res.ok) return { verified: null, partial: undefined, down: true, record: null };
    const rec = res.data;
    if (!rec) return { verified: false, partial: undefined, down: false, record: null };      // a real 404
    if (typeof rec.is_verified !== 'boolean') return { verified: null, partial: undefined, down: true, record: null };
    return {
      verified: rec.is_verified,
      partial: rec.is_partially_verified === true ? true
        : (rec.is_partially_verified === null || rec.is_partially_verified === undefined ? null : false),
      down: false,
      record: rec,
    };
  }
  ...
  async function readAddress(addr) {
    ...
    const scv = verificationOf(scRes);
    out.verified = scv.verified;
    out.partial = scv.partial;
    if (scv.down) out.explorerDown = true;
    const sc = scv.record;
    if (sc) {
    ...
    if (out.proxy && out.proxy.target) {
      ...
      // Round 21 F-1/F-2: this used to be its own, separate reading of the same question `verificationOf`
      // answers above -- `res.ok ? !!(impl && impl.is_verified) : null` -- which is why round twenty's F-2
      // fix (a 200 body with no real is_verified boolean is not an answer) and F-1 fix (a verification that
      // cannot be called full or partial is "not known", not "matched") were never applied to the code a
      // proxy actually runs. One function, one answer, for both.
      const implv = verificationOf(res);
      out.proxy.verified = implv.verified;
      out.proxy.partial = implv.partial;
      if (implv.down) out.explorerDown = true;
      const impl = implv.record;
      if (impl) {
  ...
  // The three-way wording for "how sure is this page that the published source is ALL of the code that
  // runs", shared by the contract's own pill and the proxy's (round 21 F-2: these were two copies, and only
  // the contract's had a partial state at all -- a proxy in front of a partially verified implementation
  // read as fully published with the caveat silently dropped).
  function partialSuffix(partial) {
    return partial === true ? ', partly matched'
      : partial === null ? ' (whether the match is full or partial is not known)'
      : ' and matched';
  }
  ...
  // Round twenty F-7: `approve(address,uint256)` is the same selector on ERC-721, where the number is a
  // token id and never an amount. Comparing a token id against a collection's totalSupply and calling
  // anything at or beyond it "unlimited" is wrong in both directions on a real collection -- ids are not
  // dense, so a live id at or past totalSupply is ordinary -- and describeCall already gives the honest
  // per-token sentence for exactly this call, directly above where this would otherwise contradict it.
  // increaseAllowance and permit are not part of any NFT standard, so they need no such exception.
  const unlimitedApproval = (p, target) => {
    const sh = p && p.signature ? APPROVAL_SHAPES[p.signature] : null;
    if (!sh || !p.args) return null;
    if (p.signature === 'approve(address,uint256)' && target && (target.standard === 'ERC-721' || target.standard === 'ERC-1155')) return null;
    return unlimitedFor(p.args[sh.amount], target && target.token) ? { spender: p.args[sh.spender], how: sh.how } : null;
  };
  ...
  async function renderInner(parsed, target, depth) {
    ...
    for (const c of inner) {
      ...
      rows.push(h('div', { class: 'move' }, [
        ...
        // Round 21 F-3: round twenty's F-7 fix changed unlimitedApproval's second parameter from the token
        // record to the whole readAddress record (it reads target.standard AND target.token), and updated
        // callWarnings' call site but not this one. `t.token` has neither field, so both the supply
        // comparison and the ERC-721/1155 exception were silently disabled for every call carried inside
        // another. One argument, the same one callWarnings already passes.
        unlimitedApproval(p, t)
    ...
    return card(title || 'The contract', [
      h('div', {}, [
        ...
        h('span', { class: 'pill ' + (o.proxy
          ...
          text: o.proxy
            ? (o.proxy.verified ? 'the code it runs has published source' + partialSuffix(o.proxy.partial)
              : o.proxy.verified === null ? 'could not check the code it runs'
              : 'the code it runs has published no source')
            : (o.verified ? 'source published' + partialSuffix(o.partial)
      ...
      h('div', { class: 'note warn' }, [
        ...
        // Round 21 F-2: this read o.partial unconditionally, which is the FORWARDER's own record. Behind a
        // proxy the code this section is actually built from is the implementation's, so its own partial
        // state is what has to be checked here -- the same state the pill above now reads with it.
        (function (partial) {
          return partial === true ? document.createTextNode('  This contract is only partially verified, so even the published source may not be all of the code that runs.')
            : partial === null ? document.createTextNode('  Whether this match is full or partial is not known, so even the published source may not be all of the code that runs.') : null;
        })(o.proxy ? o.proxy.partial : o.partial),
      ]),
  ...
  function callWarnings(parsed, target, sim, from) {
    const w = [];
    const unl = unlimitedApproval(parsed, target);
    if (unl)
